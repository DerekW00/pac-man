...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        ##state = [(x,y), action, cost, number explored ]
    ...
    def expand(self, state):
        ...
        for action in self.getActions(state):
            ...
            "*** YOUR CODE HERE ***"
            next_one = self.getNextState(state, action)
            cost = self.getActionCost(state, action,next_one)
            child = (next_one, action, cost)
        ...
        return children
    """
    def check_corner(self, state, nextx, nexty):
        for i in state[1]:
            if (nextx, nexty) == i:
                lsx = state[1]
                del lsx[i]
                return ((nextx, nexty), lsx)
...
def cornersHeuristic(state, problem):
    ...
    for i in state[1]:
        val.append( abs(i[0] - pos[0]) + abs( i[1] - pos[1]) )
...
def foodHeuristic(state, problem):
    ...
    if foodGrid.asList():
        for food in foodGrid.asList():
            val.append(mazeDistance(position, food, problem.startingGameState))
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        for iX, fx in enumerate(food):
            for iY, useless in enumerate(fx):
                if food[iX][iY]:
                    md = mazeDistance((x,y), (iX,iY), gameState)
                    if md < max_ind:
                        max_x = iX
                        max_y = iY
                        max_ind = md

        problem_new = PositionSearchProblem(gameState, start=(x,y), goal=(max_x,max_y), warn=False, visualize=False)
        return search.breadthFirstSearch(problem_new)
