...
def constructBayesNet(gameState):
    ...
    for i in gameState.getPossibleHouses():
        for j in gameState.getHouseWalls(i):
            obsVars.append(OBS_VAR_TEMPLATE % j)

    for i in HOUSE_VARS:
        edges.append((X_POS_VAR, i))
        edges.append((Y_POS_VAR, i))
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    "*** YOUR CODE HERE ***"
    maxProb = (0, 0)
    q = inference.inferenceByVariableElimination(
        bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder)

    for i in q.getAllPossibleAssignmentDicts():
        prob = q.getProbability(i)
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        for i in q.getAllPossibleAssignmentDicts():
            ...
            if i[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and i[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                leftExpectedValue += prob * WON_GAME_REWARD
                rightExpectedValue += prob * GHOST_COLLISION_REWARD

            elif i[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and i[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        for i, j in self.getExplorationProbsAndOutcomes(evidence):
            evidence.update(j)
            expectedValue += i * \
