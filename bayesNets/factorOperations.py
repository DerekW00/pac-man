...
def joinFactors(factors):
    ...
    "*** YOUR CODE HERE ***"
    def helper_function(left_, right_):
        left_newfactor = set.union(left_.unconditionedVariables(),
                                   right_.unconditionedVariables())
        right_newfactor = set.difference(set.union(left_.conditionedVariables(
        ), right_.conditionedVariables()), left_newfactor)
        new = Factor(left_newfactor, right_newfactor,
                     left_.variableDomainsDict())

        for assignment in new.getAllPossibleAssignmentDicts():
            left_prob = left_.getProbability(assignment)
            right_prob = right_.getProbability(assignment)
            new.setProbability(assignment, left_prob*right_prob)
        return new

    return functools.reduce(helper_function, factors)
    "*** END YOUR CODE HERE ***"
    ...
    def eliminate(factor, eliminationVariable):
        ...
        for i in factor.unconditionedVariables():
            if i != eliminationVariable:
                result.append(i)
        new = Factor(result, factor.conditionedVariables(),
                     factor.variableDomainsDict())
        for assignment in factor.getAllPossibleAssignmentDicts():
            ogProb = factor.getProbability(assignment)
            assignment.pop(eliminationVariable)
            runningSum = ogProb + new.getProbability(assignment)
            new.setProbability(assignment, runningSum)
        return new
        "*** END YOUR CODE HERE ***"
...
def normalize(factor):
    ...
    for i in factor.unconditionedVariables():
        if len(variableDomainsDict[i]) == 1:
            conditioned.add(i)
            unconditioned.remove(i)
    newFactor = Factor(unconditioned, conditioned, variableDomainsDict)
    for i in newFactor.getAllPossibleAssignmentDicts():
        sum_prob += factor.getProbability(i)
    if sum_prob == 0:
        return None
    for i in newFactor.getAllPossibleAssignmentDicts():
        newFactor.setProbability(i, factor.getProbability(i)/sum_prob)
